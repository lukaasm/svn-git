...
Activity initially displays 20 operations, with **Show more**, full-history search, and **Needs attention** / **Finished** filters. Steps and checkpoint controls load when expanded. Backup contents can be searched by branch, checkout, and shelf name; long lists scroll within the preview. Leaving Backup or a workflow preview cancels its background read, including Git processes and storage scans. Submitted operations continue in Tasks.
...
`scripts/test-browsing-responsiveness.ps1 -FixtureRoot <workflow-fixture-root> -SourceRoot <real-root>` replays 240 activity records in the disposable fixture, checks bounded rendering, lazy details, full-history search, filters, and backup search. A local TCP server deliberately stalls a Git read so navigation must close the connection. No source operation is executed; fixture configuration is restored and generated records are removed afterward. Run fixture-based UI scripts sequentially when sharing a fixture.
