...
public static class AgentNotes
{
    public static void Write(string worktree, CheckoutConfig co, IEnumerable<string> shared, SharedMode mode = SharedMode.Junction)
    {
        ...
        var server = co.IsGit ? "the server" : "SVN";
        var note = mode == SharedMode.Junction
            ? $"8. These folders are junctions into the {(co.IsGit ? "clone" : "SVN checkout")}. Do not edit files in them: " + list
        ...
        var master = co.IsGit
            ? $"""
               This folder is a git worktree managed by `sg`. The master repository is the git remote `{co.ReposRoot}`,
               branch `{co.Branch}`, and it is reached only through the clone at `{co.Path}`, never from here.
               The base ref is `svn/{co.Name}`. It mirrors that branch as the server has it.
               """
            : $"""
        ...
        var md = $"""
            ...
            3. To get the latest {server} state under your work, run `sg rebase`.
