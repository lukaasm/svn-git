...
Checkout navigation and page headers use stable, theme-aware identity colors and compact initials. Collapsed navigation keeps the initials visible, with the full name and folder in its tooltip. Checkout validation and volume checks run off the UI thread; the three folder pickers share one directory scan, and late results cannot replace newer input.
...
`scripts/test-push-message.ps1` creates three multiline commits in a local SVN fixture and verifies that the Push to SVN message follows the selected range, Send all restores the full message, and manual edits survive range changes. A fixture-only Git proxy delays or fails preview reads to check disabled actions, stable layout, cancellation of obsolete reads and inline retry. It runs through native UI Automation on a private desktop and cancels every dialog without pushing. Build the Debug CLI and app first.
