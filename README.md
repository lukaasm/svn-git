...
`scripts/test-destination-preview.ps1` checks import and backup restore at 600 px and 1280 px with native UI Automation on a private desktop. A local SVN/Git fixture verifies checkout-first fields, wrapping actions, responsive typing during delayed revision reads, cancellation when switching destinations or leaving the page, and retry after read failures. Build the Debug CLI and app first. Isolated Debug runs can set `SG_UI_TEST_WINDOW_SIZE=600x900` to test initial layout without activating or resizing a window on the user's desktop.
