...
`--worktree <name-or-path>` selects a registered worktree for these commands. `--body-file -` reads CLI stdin; MCP callers should use typed tools with a `body` string. Line comments retain their original text even after edits or deletion. Exact context may relocate within the same file; changed, ambiguous, and missing locations are reported explicitly. Rename tracking and inline gutter threads are future enhancements. Binary files, symlinks/shared folders, and text over 1 MiB are not loaded for inline review.

## UI and readiness

Expand a worktree and choose **Review code**, or launch `sg-ui code-review <worktree-path>`. Select a changed file, then use the diff's **Comment on selected lines** action or **Comment** for a whole-file/manual original-or-modified range. The comments pane provides context, replies, resolve/reopen, and an open/all filter. **Copy agent instructions** prepares a handoff. **Refresh** returns from saved thread context to the current worktree diff. Metadata is saved without editing the source or making a commit.
